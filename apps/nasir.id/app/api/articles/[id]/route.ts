...
export async function GET(_request: Request, { params }: RouteParams) {
    ...
    try {
      ...
      SELECT id, title, slug, summary, content, image_url, images, tags, is_portfolio, language, published_at, updated_at
      FROM articles
...
export async function PUT(request: Request, { params }: RouteParams) {
    ...
    try {
        ...
        const [article] = await sql`
      UPDATE articles
      SET title = ${title}, slug = ${slug}, summary = ${summary || ''}, content = ${content}, image_url = ${image_url || ''}, images = ${imagesArray}, tags = ${tagsArray}, is_portfolio = ${is_portfolio || false}, language = ${articleLanguage}, updated_at = NOW()
      WHERE id = ${parseInt(id)}
